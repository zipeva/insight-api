...
function calculateHourlyRewards() {
  ...
  for (let i = 1; i < HOURS_IN_A_DAY; ++i) {
    const decrement = getRandomReward(DECREMENT_MIN, DECREMENT_MAX);
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  this.getPreviousBlock(prevHash, function(err, info) {
    ...
    if (info === null || info.height === 0) {
      ...
      // Initialize random number generator with seed based on block height
      const seed = nPrevHeight;
      const rng = new Math.seedrandom(seed.toString());
      const distMin = INITIAL_REWARD_MIN;
      const distMax = INITIAL_REWARD_MAX;
      const randomBaseSubsidy = Math.floor(rng() * (distMax - distMin + 1)) + distMin;
      ...
      if (nPrevHeight > nBudgetPaymentsStartBlock) {
        ...
        nSuperblockPart = nSubsidy / (true /* replace with condition for v20 */ ? 5 : 10);
      }
