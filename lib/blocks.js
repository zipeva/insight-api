...
// Custom seeded random number generator function
function seededRandom(seed) {
  const mask = 0xffffffff;
  let m_w = (seed & mask) ^ 0x5bd1e995;
  let m_z = (seed * 0x6c078965) & mask;

  return function() {
    m_z = (36969 * (m_z & 0xffff) + (m_z >>> 16)) & mask;
    m_w = (18000 * (m_w & 0xffff) + (m_w >>> 16)) & mask;
    let result = ((m_z << 16) + (m_w & 0xffff)) >>> 0;
    result /= 4294967296;
    return result + 0.5;
  };
}

// Function to get a random reward between min and max using a seeded RNG
function getRandomReward(min, max, rng) {
  return Math.floor(rng() * (max - min + 1)) + min;
}

// Function to calculate rewards for 24 hours
function calculateHourlyRewards() {
  const rewards = new Array(HOURS_IN_A_DAY);

  // Get the initial reward for the first hour
  let initialReward = getRandomReward(INITIAL_REWARD_MIN, INITIAL_REWARD_MAX, Math.random);
  rewards[0] = initialReward;

  // Calculate rewards for the remaining hours
  for (let i = 1; i < HOURS_IN_A_DAY; ++i) {
    const decrement = getRandomReward(DECREMENT_MIN, DECREMENT_MAX, Math.random);
...
const hourlyRewards = calculateHourlyRewards();
...
BlockController.prototype.getBlockReward = function(prevHash, cb) {
  ...
  this.getPreviousBlock(prevHash, function(err, info) {
    ...
    if (info === null || info.height === 0) {
      ...
      // Seed the random number generator based on block height
      const rng = seededRandom(nPrevHeight);

      // Generate random base subsidy between 50 and 500 for each hour
      const hour = Math.floor(nPrevHeight / 60) % 24; // Assuming 1 block per minute, adjust if necessary
      const randomBaseSubsidy = getRandomReward(INITIAL_REWARD_MIN, INITIAL_REWARD_MAX, rng);

      nSubsidyBase = randomBaseSubsidy - Math.floor(hour * (450 / 24));
      ...
      cb(null, parseFloat(reward.toFixed(8)));
    }
  });
};
